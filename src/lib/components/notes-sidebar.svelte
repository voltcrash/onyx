<script lang="ts">
	import { Bold, Code2, Columns2, Eye, FileText, Heading2, Italic, Link, List, LoaderCircle, LogOut, PanelLeftClose, PencilLine, Plus, Quote, Search, Settings, WifiOff, Wrench, X } from '@lucide/svelte';
	...
	import type { GithubState, SaveState, TransferState, ViewMode } from './app-types';
	...
	let {
		...
		isOnline, githubState, githubUser, githubMessage, transferState, storageError, shortcuts, primaryModifier, viewMode, wordCount, readingMinutes,
		...
		onInsertSyntax, onPrefixLine, onViewModeChange, onOpenInlinePreview
	}: Props = $props();
		...
		<div class="sidebar-panel tools-panel" role="tabpanel">
			<section class="tool-section">
				...
				<div class="sidebar-view-options">
					<button class:active={viewMode === 'edit'} aria-pressed={viewMode === 'edit'} onclick={() => onViewModeChange('edit')} aria-label="Editor only"><PencilLine size={16} /><span>Edit</span></button>
					<button class:active={viewMode === 'live'} aria-pressed={viewMode === 'live'} onclick={onOpenInlinePreview} aria-label="Inline preview"><Eye size={16} /><span>Inline</span></button>
					<button class:active={viewMode === 'split'} aria-pressed={viewMode === 'split'} onclick={() => onViewModeChange('split')} aria-label="Split view"><Columns2 size={16} /><span>Split</span></button>
					<button class:active={viewMode === 'preview'} aria-pressed={viewMode === 'preview'} onclick={() => onViewModeChange('preview')} aria-label="Preview only"><Eye size={16} /><span>Preview</span></button>
				</div>
			</section>
			<section class="tool-section">
				<h2>Formatting</h2>
