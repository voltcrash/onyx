<script lang="ts">
	...
	import {
		...
		type NoteMetadata, type ShortcutAction, type ThemePreference, type VaultChangeEvent, type VaultSearchResult
	} from '$lib';
...
Press \`${commandPaletteShortcut}\` for the command palette, \`${saveShortcut}\` to save now, or \`${previewShortcut}\` to toggle preview. Press \`?\` for every shortcut.`;
	...
	async function flushDrafts(): Promise<boolean> {
		while (saveRequested) {
			...
			saveState = 'saving';
			try {
				const savedNote = await vault.saveNote({ id: noteId, title: titleFromMarkdown(contents), markdown: contents, expectedRevision });
