...
Press \`${commandPaletteShortcut}\` for the command palette, \`${saveShortcut}\` to save now, or \`${previewShortcut}\` to toggle preview. Press \`?\` for every shortcut.`;
  ...
  const paletteItems = $derived([
    ...
    },
    {
      ...
      group: "View",
      label:
        sidebarSide === "left" ? "Move the sidebar to the right" : "Move the sidebar to the left",
      icon: PanelLeft,
      keywords: "panel side position dock swap",
      run: () => setSidebarSide(sidebarSide === "left" ? "right" : "left"),
    },
    {
      id: "theme-light",
      group: "Themes",
